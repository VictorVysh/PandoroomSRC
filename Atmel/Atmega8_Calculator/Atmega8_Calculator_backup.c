...
void calculation(char ch)
{
	switch(MODE)
	{
		case 0:
			...
			{
				LINE_L_MINUS;
				A_VAL = A_VAL/10;//soft_div(A_VAL,10);
				LCD_BACKSPACE;
			}
			else if(ch=='+'||ch=='-'||ch=='/'||ch=='*')
			{
			...
			{
				LINE_L_PLUS;
				A_VAL = A_VAL*10+(ch-'0');//soft_multp(A_VAL,10)+(ch-'0');
				lcd_write_char(ch);
			}
		break;
		case 1:
			...
			{
				LINE_L_MINUS;
				B_VAL = B_VAL/10;//soft_div(B_VAL,10);
				LCD_BACKSPACE;
			}
			else if(ch=='=')
			{
				switch(OPERATION_VAL)
				{
					...
					break;
					case '/':
					RESULT = A_VAL/B_VAL;//soft_div(A_VAL,B_VAL);
					break;
					case '*':
					RESULT = A_VAL*B_VAL;//soft_multp(A_VAL,B_VAL);
					break;
				}
				LINE_L_PLUS;
				lcd_write_char(ch);
				sprintf(RESULT_CH,"%f",RESULT);//itoa(RESULT,RESULT_CH,10);
			...
			{
				LINE_L_PLUS;
				B_VAL = B_VAL*10+(ch-'0');//soft_multp(B_VAL,10)+(ch-'0');
				lcd_write_char(ch);
			}
		break;
		case 2:
